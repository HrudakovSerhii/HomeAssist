...
export class TemplateValidatorService {
  ...
  validateTemplate(template: EmailAnalysisTemplate): TemplateValidationResult {
    ...
    example.entities.forEach((entity, index) => {
      ...
      if (entity.confidence < 0 || entity.confidence > 1) {
        errors.push(`Invalid confidence value at entity index ${index}: ${entity.confidence}`);
      }
    });
    ...
    example.actionItems.forEach((action, index) => {
      if (!validActionTypes.includes(action.actionType)) {
        errors.push(`Invalid action type at index ${index}: ${action.actionType}`);
      }
      if (!validPriorities.includes(action.priority)) {
        errors.push(`Invalid action priority at index ${index}: ${action.priority}`);
      }
    });
  ...
  private checkEnumCoverage(templateString: string): string[] {
    ...
    const mentionedCategories = Object.values(EmailCategory).filter(
      cat => templateString.includes(cat)
    );
    if (mentionedCategories.length < Object.values(EmailCategory).length) {
      warnings.push('Template does not mention all available EmailCategory values');
    }

    // Check if template mentions all entity types
    const mentionedEntityTypes = Object.values(EntityType).filter(
      type => templateString.includes(type)
    );
    if (mentionedEntityTypes.length < Object.values(EntityType).length) {
      warnings.push('Template does not mention all available EntityType values');
    }

    // Check if template mentions all action types
    const mentionedActionTypes = Object.values(ActionType).filter(
      type => templateString.includes(type)
    );
    if (mentionedActionTypes.length < Object.values(ActionType).length) {
      warnings.push('Template does not mention all available ActionType values');
    }
  ...
  async validateLLMResponse(response: any): Promise<TemplateValidationResult> {
    ...
    if (response.entities && Array.isArray(response.entities)) {
      const approvedTypes = await this.dynamicEntityManager.getAllApprovedEntityTypes();
      ...
      for (let index = 0; index < response.entities.length; index++) {
        ...
        if (!approvedTypes.includes(entity.type)) {
          ...
          const result = await this.dynamicEntityManager.processNewEntityType(suggestion);
          
          if (result.accepted) {
            warnings.push(`Entity type at index ${index}: ${entity.type} -> ${result.reason}`);
            // Update entity type if it was mapped
            if (result.mappedTo) {
              response.entities[index].type = result.mappedTo;
            }
          } else {
            errors.push(`Invalid entity type at index ${index}: ${entity.type} - ${result.reason}`);
          }
        }
    ...
    if (response.actionItems && Array.isArray(response.actionItems)) {
      response.actionItems.forEach((action: any, index: number) => {
        ...
        if (action.priority && !Object.values(Priority).includes(action.priority)) {
          errors.push(`Invalid action priority at index ${index}: ${action.priority}`);
        }
      });
    }
   ...
   */
  validateLLMResponseSync(response: any): TemplateValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    // Validate entities if present (without dynamic handling)
    if (response.entities && Array.isArray(response.entities)) {
      response.entities.forEach((entity: any, index: number) => {
        if (!Object.values(EntityType).includes(entity.type)) {
          warnings.push(`Unknown entity type at index ${index}: ${entity.type} (will be processed dynamically)`);
        }
        if (typeof entity.confidence !== 'number' || entity.confidence < 0 || entity.confidence > 1) {
          errors.push(`Invalid confidence value at entity index ${index}: ${entity.confidence}`);
        }
